...
| 0027 | [`0027_correlate_sweep_join_by_time.sql`](../supabase/migrations/0027_correlate_sweep_join_by_time.sql) | Correlates the sweep's `net._http_response` lookup by time as well as by id. `pg_net`'s request ids restart if the extension is recreated, and `supabase_functions.hooks` is never purged — so an id alone could hand an old hook a newer delivery's outcome, and report a failure as `delivered`. Paste-and-run; needs 0026 |
...
All four have to pass. Currently: 847 tests green, typecheck clean, lint
